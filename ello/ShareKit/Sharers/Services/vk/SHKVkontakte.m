...
- (id)init
{
	if (self = [super init])
	{		
		self.consumerKey = SHKVkontakteConsumerKey;		
		self.secretKey = @"b5933f82fd830eb4b3c4b98b9fb3bd56244b399b399c974e94c844d66af8e31";//SHKVkontakteSecretKey;
 		self.authorizeCallbackURL = [NSURL URLWithString:SHKVkontakteCallbackUrl];
		
		// -- //
        
        // http://api.vkontakte.ru/oauth/authorize?client_id=APP_ID&scope=SETTINGS&redirect_uri=REDIRECT_URI&display=DISPLAY&response_type=token
		
        NSString *urlTemplateString = @"http://api.vkontakte.ru/oauth/authorize?client_id=%@&scope=%@&redirect_uri=%@&display=%@&response_type=token";
        
        NSString *urlString = [NSString stringWithFormat:urlTemplateString, 
                               SHKVkontakteAppId, 
                               @"wall", 
        ...
        NSLog(@"VKontacte urlString: %@", urlString);
        
		// Edit these to provide the correct urls for each oauth step
        self.authorizeURL = [NSURL URLWithString:@"http://api.vkontakte.ru/oauth/authorize"];
	    self.requestURL = [NSURL URLWithString:@"http://api.vkontakte.ru/oauth/authorize"];
	    self.accessURL = [NSURL URLWithString:@"http://api.vkontakte.ru/oauth/authorize"]; 
